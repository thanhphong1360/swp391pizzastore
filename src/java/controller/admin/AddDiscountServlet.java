...
public class AddDiscountServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            response.sendRedirect("discountList"); // chuyển về trang danh sách
        } catch (Exception e) {
            e.printStackTrace();
            request.setAttribute("error", "Add discount failed: " + e.getMessage());
            request.getRequestDispatcher("/${pageContext.request.contextPath}/views/admin/discount/add-discount.jsp").forward(request, response);
        }
    }
